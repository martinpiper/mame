	...
	switch(reg)
	{
		...
		case 3:
			{
				...
				if (m_cur_slot >= 0 && slot.m_playing)
				{
					...
					setPitch(theTime, theChannel, slot.m_pitchForStep);
				}
			}
			break;
		case 4: // KeyOn/Off
		{
			...
			if (data & 0x80) // KeyOn
			{
				...
				{
					int theChannel = mChannelOffset + m_cur_slot;
					setNoteOn(theTime, theChannel, mSampleAddressOffset + slot.m_sample.m_start, mSampleAddressOffset + slot.m_sample.m_start + slot.m_sample.m_end, mSampleAddressOffset + slot.m_sample.m_start + slot.m_sample.m_loop, mSampleAddressOffset + slot.m_sample.m_start + slot.m_sample.m_end, slot.m_pitchForStep, slot.m_dest_total_level, slot.m_pan);
				}
			}
			else
			{
				if (slot.m_playing)
				{
					if (slot.m_sample.m_release_reg != 0xf)
					{
						...
						{
							int theChannel = mChannelOffset + m_cur_slot;
							setNoteRelease(theTime, theChannel);
						}
		...
		case 5: // TL + Interpolation
			...
			if (m_cur_slot >= 0 && slot.m_playing)
			{
				...
				setVolume(theTime , theChannel , slot.m_dest_total_level);
			}
			break;
